...
op_list = [3,3,1105,-1,9,1101,0,0,12,4,12,99,1]#[3,21,1008,21,8,20,1005,20,22,107,8,21,20,1006,20,31,1106,0,36,98,0,0,1002,21,125,20,4,20,1105,1,46,104,999,1105,1,46,1101,1000,1,20,4,20,1105,1,46,98,99]

# for line in open("opList.csv"):
# 	op_list.append(line.strip().split(","))

master = {}
for pos, x in enumerate(op_list):
	master[pos] = int(x)

op_lens = {1: 3, 2: 3, 3: 1, 4: 1, 5: 2, 6: 2, 7: 3, 8: 3, 99: 0}
...
def retrieve(op_dict, current, instr_len=5, op_code_len=2):
	...
	# vals = [
	# 	op_dict[current + i] if int(modes[i]) == 1 else op_dict[op_dict[current + i]]
	# 	for i in range(0, op_len - 1)
	# ]
	...
	if op_code == 1:
		...
		if bool(vals[0]):
			print(f"Jumping to {out_reg}")
			return op_dict, out_reg
		else:
			return op_dict, current + op_len
	elif op_code == 6:
		if bool(vals[0]):
